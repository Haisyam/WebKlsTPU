...
<body>
  ...
  $sql = "SELECT * FROM jadwal_piket
          ...
          WHERE jadwal_piket.id_piket = '$id'";
    ...
    <form method="POST">
      <div class="pb-5">
        ...
        <select name="nama_hari" id="nama_hari" class="select select-bordered w-full max-w-xs">
          ...
          ?>
            <option <?= ($data['id_hari'] == $hari['id_hari']) ? 'selected' : '' ?> value="<?= $hari['id_hari'] ?>"><?= $hari['nama_hari'] ?></option>
          <?php endwhile; ?>
        </select>
      </div>
      <div class="pb-5">
        ...
        <select name="id_siswa" id="id_siswa" class="select select-bordered w-full max-w-xs">
          ...
          ?>
            <option <?= ($data['id_siswa'] == $nama_siswa['id_siswa']) ? 'selected' : '' ?> value="<?= $nama_siswa['id_siswa'] ?>"><?= $nama_siswa['nama'] ?></option>
          <?php endwhile; ?>
        </select>
      </div>
      <div class="">
...
if (isset($_POST['submit'])) {
  ...
  $sql = "UPDATE jadwal_piket SET id_siswa = '$id_siswa', id_hari = '$nama_hari' WHERE id_piket = $id";
  $query = mysqli_query($conn, $sql);
  if ($query > 0) {
